...
function CreatorProfileContent() {
  ...
  const loadCreatorWorks = useCallback(async () => {
    if (!isConnected || !creatorAddress) return;
    setLoading(true);
    try {
  ...
  if (!creatorAddress) {
    return (
      <div className="glass-card p-12 text-center">
        <p className="text-slate-400">No creator address provided.</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="flex items-center gap-4 mb-8">
        <div className="h-16 w-16 rounded-full bg-brand-500/20 border border-brand-500/50 flex items-center justify-center shrink-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-slate-400 font-mono break-all truncate">
            {creatorAddress}
          </p>
        </div>
      </div>
